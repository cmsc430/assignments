...
(define (parse-prog s fs xs ys gs)
  (match s
    ...
    [(cons s ss)
     (match s
       [(cons 'define sr)
        (match (parse-define/acc sr fs xs ys gs)
          [(list ys gs (and d (Defn f _ _ _ _)))
...
(define (parse-define/acc s fs xs ys gs)
  (define (rec/p sr fs ps)
    (match sr
      [(list (cons (? symbol? p) sr) sb)
       (if (member p ps)
           (error "define: duplicate parameter" p)
           (rec/p (list sr sb) fs (cons p ps)))]
      [_ (rec/d sr fs (reverse ps) '() ys gs '())]))
  (define (rec/d sr fs ps ds ys gs es)
    (match sr
      [(list (list) sb)
       ;; ps reversed by rec/p, ds built in order
       (list ys gs ps ds (reverse es) sb)]
      [(list (cons (list (? symbol? d) s) sr) sb)
       (if (or (member d ds)
               (member d ps))
           (error "define: duplicate parameter" d)
           (let ((ds (append ds (list d))))
             (match (parse-e/acc s fs (append ps ds xs) ys gs)
               [(list ys gs e)
                (rec/d (list sr sb) fs ps ds ys gs (cons e es))])))]
      [_ (error "parse error" sr)]))
  (match s
    [(list (cons (? symbol? g) sr) sb)
     (match (rec/p (list sr sb) (cons g fs) '())
       [(list ys gs ps ds es s)
        (match (parse-e/acc s (cons g fs) (append ps ds xs) ys gs)
          [(list ys gs e)
           (list ys gs (Defn g ps ds es e))])])]))
