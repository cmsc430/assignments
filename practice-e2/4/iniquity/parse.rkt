...
;; S-Expr [Listof Id] [Listof Id] [Listof Id] [Listof Id] -> (list [Listof Id] [Listof Id] Expr)
;;   s: partial let expression shaped s-expr to be parsed
;;   fs: defined function names
;;   xs: bound variables
;;   ys: free variables
;;   gs: undefined function names
;; returns list of free variables, undefined function names, and parsed expression
(define (parse-let s fs xs ys gs)
  ;; S-Expr [Listof Id] [Listof Id] [Listof Expr] -> (list [Listof Id] [Listof Id] Expr)
  ;;   sr: partial let expression shaped s-expr to be parsed
  ;;   l-xs: variables bound by this let
  ;;   ys: free variables
  ;;   gs: undefined function names
  ;;   es: parsed right-hand-side expressions in this let
  ;; returns list of free variables, undefined function names, and parsed expression
  (define (rec sr l-xs ys gs es)
    (match sr
      [(list (list) sb)
       (match (parse-e/acc sb fs (append l-xs xs) ys gs)
         [(list ys gs e)
          (list ys gs (Let (reverse l-xs) (reverse es) e))])]
      [(list (cons (list (? symbol? x) s1) sr) sb)
       (if (member x l-xs)
           (error "let: duplicate variable" x)
           (match (parse-e/acc s1 fs xs ys gs)
             [(list ys gs e)
              (rec (list sr sb) (cons x l-xs) ys gs (cons e es))]))]
