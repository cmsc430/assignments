...
(begin ; Iniquity
  ...
  (check-equal? (parse-closed '(define (f x) (g x))
                              '(define (g x) (f x))
                              '(f 0))
                (Prog (list (Defn 'f '(x) (App (Var 'g) (list (Var 'x))))
                            (Defn 'g '(x) (App (Var 'f) (list (Var 'x)))))
...
(begin ; Loot with Exceptions
  (check-equal? (parse '(with-handlers ([f h]) (raise 1)))
                (p (WithHandler (Var 'f) (Var 'h) (Raise (Lit 1))))))
