...
(begin ; Hoax+
  (check-equal? (parse '(length '())) (p (Prim1 'length (Lit '()))))
  (check-equal? (parse '(reverse '())) (p (Prim1 'reverse (Lit '()))))
  (check-equal? (parse '(list->vector '())) (p (Prim1 'list->vector (Lit '()))))
  (check-equal? (parse '(vector->list '())) (p (Prim1 'vector->list (Lit '())))))
