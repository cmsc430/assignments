...
(define (test run)
  ...
  (begin ;; Hoax+
    ...
    (check-equal? (run '(length (cons 1 '()))) 1)
    (check-equal? (run '(reverse '())) '())
    (check-equal? (run '(reverse (cons 1 '()))) '(1))
    (check-equal? (run '(list->vector '())) '#())
    (check-equal? (run '(list->vector (cons 1 '()))) '#(1))
    (check-equal? (run '(vector->list (make-vector 0 1))) '())
    (check-equal? (run '(vector->list (make-vector 1 1))) '(1))))
