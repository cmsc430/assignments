...
(define (parse-match-pattern/acc s fs xs ys gs)
  (define (rec p xs ys gs)
    (match p
      ...
      [(cons 'list sr)
       (match (parse-match-patterns/acc sr fs xs ys gs)
         [(list ys xs gs ps)
          (list ys xs gs (List ps))])]
      [(cons 'vector sr)
       (match (parse-match-patterns/acc sr fs xs ys gs)
         [(list ys xs gs ps)
          (list ys xs gs (Vect ps))])]
      [(list '? (? symbol? f))
       (if (memq f fs)
           (list ys xs gs (Pred f))
           (list ys xs (cons f gs) (Pred f)))]
      [_ (error "parse pattern error")]))
  (rec s xs ys gs))

;; S-Expr [Listof Id] [Listof Id] [Listof Id] [Listof Id] -> (list [Listof Id] [Listof Id] [Listof Id] [Listof Pat])
(define (parse-match-patterns/acc sr fs xs ys gs)
  (match sr
    ['() (list ys xs gs '())]
    [(cons s sr)
     (match (parse-match-pattern/acc s fs xs ys gs)
       [(list ys xs gs p)
        (match (parse-match-patterns/acc sr fs xs ys gs)
          [(list ys xs gs ps)
