...
(begin ; Knock+
  (check-equal? (parse '(match 1 [(? f?) #t]))
                (p (Match (Lit 1) (list (Pred 'f?)) (list (Lit #t)))))
  (check-equal? (parse '(match x [(list) 1]))
                (p (Match (Var 'x) (list (List (list))) (list (Lit 1)))))
  (check-equal? (parse '(match x [(list p q s) p]))
                (p (Match (Var 'x) (list (List (list (Var 'p) (Var 'q) (Var 's)))) (list (Var 'p)))))
